...
Trace detail includes a request snapshot for replay assistance. It stores model, messages, metadata and data labels, but never stores app tokens. When `data_labels` contains `sensitive`, message content and metadata values are stored as `[redacted]`; the structure and labels remain available for troubleshooting.
